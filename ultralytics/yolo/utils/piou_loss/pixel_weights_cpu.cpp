...
at::Tensor pixel_weights_forward_cpu(const at::Tensor &loc_p,   //
                                    ...
                                    const int k)
{
    TORCH_CHECK(grid_xy.ndimension() == 2, "Feature should be NxC forms");
    TORCH_CHECK(loc_t.ndimension() == 2, "Feature should be NxC forms");
    TORCH_CHECK(loc_p.ndimension() == 2, "Feature should be NxC forms");
    TORCH_CHECK(grid_xy.is_contiguous(), "Feature should be contiguous");
    TORCH_CHECK(loc_p.is_contiguous(), "Feature should be contiguous");
    TORCH_CHECK(loc_t.is_contiguous(), "Feature should be contiguous");

...
//     }
        ...
        at::Tensor &grad_loc_memory)
    {
        ...
        TORCH_CHECK(loc_p.device().index() == loc_t.device().index(), "loc_p & loc_t must be same device");
        TORCH_CHECK(loc_p.device().index() == grid_x.device().index(), "loc_p & grid_x must be same device");
        ...
        TORCH_CHECK(cudaGetLastError() == cudaSuccess, "pious_forward_kernel failed");
        return output;
    }
        ...
        const at::Tensor &grid)
    {
        ...
        // TORCH_CHECK(cudaGetLastError() == cudaSuccess, "overlap_r_forward_kernel failed");
        return output;
    }
    /* ------------------------------end of the forward--------------------------- */
    ...
    at::Tensor pixel_weights_backward_cuda(const at::Tensor &grads_pious, const at::Tensor &grad_loc_memory)
    {
        ...
        TORCH_CHECK(cudaGetLastError() == cudaSuccess, "pious_backward_kernel failed");
        return grad_loc_p;
    }
